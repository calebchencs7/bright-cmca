# run_unet_dacutmix.py -- UNet + DACutMix (second-innovation ablation)
# =====================================================================
# Isolates the contribution of DACutMix (Damage-Aware CutMix) without
# CMCA, completing the ablation matrix:
#
#   UNet                      : baseline (no CMCA, no DACutMix)
#   UNet + CMCA               : first contribution only
#   UNet + DACutMix          : THIS SCRIPT -- second contribution only
#   UNet + CMCA + DACutMix   : full method (see run_unet_cmca_dacutmix.py)
#
...
#
# Same training budget, optimizer, and DACutMix hyper-parameters as the
# main run so the only variable removed is the CMCA backbone.
#
import os
import sys
import subprocess
...
SAVE_DIR = os.path.join(ROOT, "checkpoints", "unet_dacutmix")
...
run([
    ...
    # Training -- identical budget to UNet baseline and to UNet+CMCA+DACutMix
    "--train_batch_size", "16",
    "--eval_batch_size", "4",
    "--num_workers", "16",
    "--crop_size", "640",
